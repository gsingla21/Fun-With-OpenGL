...
#define BLACK 0
using namespace std;
int boundaryCol[] = {255,255,0}; //color of boundary of polygon
int fillColor[] = {0,0,255}; //color to be filled inside the polygon
...
glReadPixels(x_curr, y_curr, 1, 1, GL_RGB,
GL_UNSIGNED_BYTE, current_pixel_color);
red_val = current_pixel_color[0];
green_val = current_pixel_color[1];
blue_val = current_pixel_color[2];
if( !((red_val == 255) && (green_val == 255) && (blue_val ==
0)) &&
!((red_val == 0) && (green_val == 0) && (blue_val ==
255))&&
(x_curr<=300 && x_curr>=100) && (y_curr<=300 &&
y_curr>=100))
return true;
return false;
}
